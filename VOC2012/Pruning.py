...
try:
    ...
    #     filepath=os.path.join(TRAIN_CHECKPOINT, "checkpoint_p.h5"), save_best_only=True)
    ...
    'pruning_schedule': tfmot.sparsity.keras.PolynomialDecay(initial_sparsity=0.0,
                                                            final_sparsity=0.70,
                                                            begin_step=0,
                                                            end_step=int(TRAIN_DATASET_SIZE / BATCH_SIZE))
    }
    ...
    try:
        # Train model
        print("\n\nTrain Model...")
        model_for_pruning.compile(loss=Model.weighted_SparseCategoricalCrossentropy(SAMPLE_WEIGHT, classes=LABELS), optimizer='adam', metrics=[tf.keras.metrics.SparseCategoricalAccuracy()])

        model_for_pruning.fit(train_dataset, validation_data=test_dataset, epochs=EPOCHS,
    ...
    try:
        ...
        model_for_export.save('Model_V0_1_p.h5', include_optimizer=False)
        print("  Done\n\n")
    except:
        import traceback
        traceback.print_exc()
